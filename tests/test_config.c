...
int main(void) {
	...
	TEST_ASSERT_STR_EQ("https://updates.example.com/enroll",
			   cfg.enroll_url);
	TEST_ASSERT_STR_EQ("/etc/ota-fetch/ca.pem", cfg.tls_ca_cert);
	TEST_ASSERT_STR_EQ("/etc/ota-fetch/client.crt", cfg.tls_client_cert);
	TEST_ASSERT_STR_EQ("/etc/ota-fetch/client.key", cfg.tls_client_key);
	...
	TEST_ASSERT_STR_EQ("/var/lib/ota-fetch/inbox", cfg.inbox_manifest_dir);
	TEST_ASSERT_STR_EQ("/var/lib/ota-fetch/current",
			   cfg.current_manifest_dir);
	TEST_ASSERT_STR_EQ("/etc/ota-fetch/root_ca.pem", cfg.manifest_ca_cert);
