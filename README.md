...
1. **Transport security (HTTPS/mTLS)**
   ...
   - `tls_ca_cert` is used to validate the HTTPS server certificate.
   - `tls_client_cert` and `tls_client_key` are presented for mTLS.
...
2. **Manifest authenticity**
   ...
   - The signer certificate must chain to `manifest_ca_cert` (file or hashed CA
...
- **Signature verification failure**: signer certificate does not chain to
  `manifest_ca_cert`, or `manifest.json.sig` is invalid.
...
Primary tests are CMake + CTest based.
...
Legacy HTTPS/mTLS integration scripts remain under `test/scripts/` for manual
validation, but they are no longer the primary test workflow.

## CI

GitHub Actions workflow: `.github/workflows/ci.yml`

- Runs compiler matrix builds (GCC and Clang) on Ubuntu
- Builds with CMake and runs CTest
- Runs clang-format checks
- Runs ASan/UBSan CTest job
...
├── tests/              # CTest unit/integration-style offline tests
├── test/               # Legacy manual integration scripts/fixtures
